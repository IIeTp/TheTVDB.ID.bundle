...
class id_TVDBAgent(Agent.TV_Shows):
...
##############################################################################################################################################################
    ...
    # Begin MOD_ID
    # Looking for "tvdbdddd" or "tvdb-dddd" or tvdb_dddd in the series name, directory name, FileBot XATTR metadata, and use it.
    ...
    if file:
      ...
      if idtvdb is not None:
        Log('Searching series \"'  + media.show + '\" by TVDB ID \'' + idtvdb + '\'')
        try:
          series_data = JSON.ObjectFromString(GetResultFromNetwork(TVDB_SERIES_URL % (idtvdb, lang), additionalHeaders={'Accept-Language': lang}))['data']
          self.ParseSeries(media, series_data, lang, results, 250)
        except Exception, e:
      ...
      if id_series is not None:
        ...
        # else:
          # pass
      id_series = id_movie = find_id(r'(?<=tt)\d{7,}', search_string)
      if id_series is not None:
        Log('Searching Series TVDB ID \"'  + xstr(media.show) + '\" by IMDB ID \'' + id_series + '\'')
        tmdb_dict = GetTMDBJSON(url=TMDB_TV_IMDB % ('tt' + id_series))
        if isinstance(tmdb_dict, dict):
          tmdb_id = find_id(r'(?<=\'id\': )\d+', xstr(tmdb_dict['tv_results']))
          tmdb_dict = GetTMDBJSON(url=TMDB_TV_EXTERNAL_IDS % (tmdb_id))
          if isinstance(tmdb_dict, dict):
            idtvdb = find_id(r'(?<=\'tvdb_id\': )\d+', str(tmdb_dict)) 
            if idtvdb is not None:
